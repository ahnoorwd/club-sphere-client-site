...
import { useQuery, useQueryClient } from "@tanstack/react-query";
import Swal from "sweetalert2";
import { getEventById, getEventCapacity } from "../api/events";
import { AuthContext } from "../Authprovider/AuthProvider";
import {
  checkEventRegistration,
  registerEvent,
...
const EventDetails = () => {
  ...
  const { data: capacity = {} } = useQuery({
    ...
    enabled: !!id,
    queryFn: async () => {
      return await getEventCapacity(id);
    },
  });
  ...
  const handleRegisterEvent = async () => {
    ...
    try {
      ...
      if (result.full) {
        ...
        queryClient.invalidateQueries(["eventCapacity", id]);
        return;
      }
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="bg-gradient-to-br from-cyan-500 via-blue-500 to-violet-500 rounded-2xl p-6 text-white flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-white/90 leading-7">
                    Register for this event and become part of the
                    ClubSphere community experience.
                  </p>
                </div>
                ...
                <button
                  onClick={handleRegisterEvent}
                  disabled={
                    registrationStatus.registered || capacity.isFull
                  }
                  className={`btn mt-6 rounded-full border-none ${
                    registrationStatus.registered || capacity.isFull
                      ? "bg-slate-200 text-slate-500 cursor-not-allowed"
